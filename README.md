# Hi, I'm Stewart Nyamutswa 👋

### Cybersecurity Graduate | SOC Analyst | Cloud Security | AI Security Automation

I'm a cybersecurity graduate passionate about building practical, enterprise-inspired security solutions that address real-world cybersecurity challenges. My work focuses on Security Operations (SOC), Cloud Security, Identity & Access Management (IAM), Vulnerability Management, Incident Response, and AI-powered Security Automation.

This GitHub profile showcases my cybersecurity journey through hands-on projects, professional certifications, technical documentation, and demonstration videos. Each project is designed to simulate real enterprise environments and demonstrate practical experience using industry-standard security technologies.

---

# 👨‍💻 About My Work

Here you'll find:

- 🚀 Enterprise-style cybersecurity projects
- ☁️ Cloud security architectures on AWS
- 🛡 SOC investigation and security automation workflows
- 🔐 Identity & Access Management (Active Directory & AWS IAM)
- 🔍 Vulnerability management and remediation platforms
- 📜 Professional cybersecurity certifications
- 💻 Technical documentation and source code
- 🎥 Project demonstration videos

---

# 🛡 Featured Cybersecurity Projects

| # | Project | Focus | Demo |
|---|---|---|---|
| 1 | [Automated Hybrid SOC Incident Detection & Response Platform](https://github.com/smnyamutswa/Cybersecurity-Portfolio/blob/main/01-Automated-Hybrid-SOC/README.md) | SIEM, SOAR, Alert Triage, Case Management, Threat Intelligence | |
| 2 | [Hybrid Identity Risk Analyzer with Agentic AI](https://github.com/smnyamutswa/Cybersecurity-Portfolio/blob/main/02-Hybrid-Identity-Risk-Analyzer/README.md) | Active Directory, AWS IAM, AI Investigation, Risk Analysis | 🎥 Coming Soon |
| 3 | [AI-Assisted Vulnerability Management Platform](https://github.com/smnyamutswa/Cybersecurity-Portfolio/blob/main/03-AI-Vulnerability-Management/README.md) | OpenVAS, ServiceNow, Automated Remediation, Verification | 🎥 Coming Soon |
| 4 | [Secure Cloud-Native E-Commerce Platform on AWS](https://github.com/smnyamutswa/cybersecurity-portfolio/tree/main/04-Secure-AWS-ECommerce-Platform) | AWS Security Architecture, Monitoring, Infrastructure as Code | 🎥 Coming Soon |
| 5 | [AI-Powered SOC Investigation & Alert Enrichment Engine](https://github.com/smnyamutswa/cybersecurity-portfolio/tree/main/05-AI-SOC-Investigation-Engine) | Splunk, Threat Intelligence, AI Investigation, ServiceNow | 🎥 Coming Soon |
| 6 | [ACTIVE DIRECTORY PROJECT](https://github.com/smnyamutswa/Active-Directory) | ACTIVE DIRECTORY, User and Group Management, Group Policy Configuration, DNS Integration|  |) 

---

# 💻 Core Skills

### 🛡 Security Operations
- SIEM & SOAR
- Alert Detection, Triage & Investigation
- Incident Response
- Threat Intelligence
- Detection Engineering

### ☁️ Cloud Security
- AWS Security Architecture
- Identity & Access Management (IAM)
- Cloud Monitoring & Logging
- Infrastructure as Code (CloudFormation)

### 🔐 Identity Security
- Active Directory
- AWS IAM
- Privileged Access Analysis
- Identity Risk Assessment

### 🔍 Vulnerability Management
- Vulnerability Assessment
- Risk Prioritization
- Remediation Workflows
- Verification & Reporting

### ⚙️ Programming & Automation
- Python
- REST APIs
- Security Automation
- Agentic AI Workflows

---

# 🏆 Certifications

<a href="https://www.credly.com/badges/1a0a6769-93c4-4f6d-a4f8-7cdc74809d3e/public_url">
<img src="https://img.shields.io/badge/AWS_Security_Specialty-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white"/>
</a>
&nbsp;
<a href="https://www.credly.com/badges/13ecd1ea-4222-475b-b498-ee6d382d2308/public_url">
<img src="https://img.shields.io/badge/AWS_CloudOps_Associate-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
</a>
&nbsp;
<a href="https://www.credly.com/badges/59705383-76ad-48e6-b3cf-3b36c4588f24/public_url">
<img src="https://img.shields.io/badge/CompTIA_CySA+-E2231A?style=for-the-badge&logo=comptia&logoColor=white"/>
</a>
&nbsp;
<a href="https://www.credly.com/badges/59f58920-6728-4fe4-b338-f845493c30d9/public_url">
<img src="https://img.shields.io/badge/CompTIA_Security+-CC0000?style=for-the-badge&logo=comptia&logoColor=white"/>
</a>

---

# 🎓 Education

**Master of Science in Cybersecurity**  
**Yeshiva University – Katz School of Science and Health**

**Bachelor of Science in Telecommunications Engineering**  
**Midlands State University**

---

# 🎥 Project Demonstrations

Project demonstration videos walk through the architecture, implementation, and security workflows behind each project.

📺 **YouTube Portfolio**  
*(Add your YouTube playlist link here.)*

---

# 🤝 Connect With Me

💼 **LinkedIn**  
https://www.linkedin.com/in/YOUR-LINKEDIN

📂 **Cybersecurity Portfolio**  
https://github.com/smnyamutswa/cybersecurity-portfolio

🎥 **YouTube**  
https://www.youtube.com/@StewartNyamutswa/playlists

📧 **Email**  
smnyamutswa@gmail.com
